...
public class PollController {
    ...
    public Response vote(@PathVariable Integer quizId, @RequestParam Integer answerId, HttpSession session) {
        return pollService.vote(quizId, answerId, authService.currentUserOrNull(session).getId());
    }

    @GetMapping("/api/{quizId}/results")
    @ResponseBody
    public Response getResults(@PathVariable Integer quizId) {
        return pollService.getPollResults(quizId);
    }
}
